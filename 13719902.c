...
void separaNumero(int valor){
    ...
    if (valor < 0 || valor > 9999){
      ...
      centenas = -1;
      milhares = -1;}
/*se o valor informado for menor que zero ou maior que 9999, a funcao retornara -1 em todas as variaveis, do contrario, ela ira informar a casa ocupada por cada algarismo*/ 
      else {
        (unidades = valor%10);
        (dezenas = (valor%100)/10);
        (centenas = (valor%1000)/100);
        (milhares = (valor%10000)/1000);}

/* a função utiliza o metodo do resto da divisao e da divisao para conseguir separar as casas do numero*/
...
/*
	...
	Se os dois parÃ¢metros valerem zero, a funcao devera retornar 0 (apenas 
...
double potenciacao(double base, int expoente){
  ...
  /* se o expoente for igual a zero e a base tambem ou se o expoente for menor que zero, a funcao retona zero, agora se o expoente for igual a zero e a base diferente de zero, o retorno sera 1 */
...
int resultado = 1; double mult = 1;

/*foi criada a variavel resultado e mult para construir o while. o laco tem como objetivo susbtituir o pow. a funcao multiplica e guarda a multiplicacao ate chegar ao expoente.*/
...
int passosDaDivisao(double dividendo, double divisor, double limiar){
    ...
    if (dividendo <= 0 || divisor <= 1 || limiar < 0){
      return -1;}
        if (limiar > dividendo){
          return 0;}
...
/*
	...
	Esta funcao recebe um numero inteiro como parÃ¢metro (n).
	...
	atual (apÃ³s tres iteracoes), conforme exemplificado.
